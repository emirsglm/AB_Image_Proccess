...
def main(args):
    ...
    #%% section 3
        model = cv2.dnn.readNetFromDarknet(str(config_path),str(weights_path))
...
def parse_args():
    parser = argparse.ArgumentParser(description='YOLO Application Help ')
    parser.add_argument("-i", "--input",
                  help="Path to input stream", required=True)
    
    parser.add_argument("-c", "--config",
                  help="yolo.cfg path", required=True)
    parser.add_argument("-w", "--weights",
                  help="yolo.weights path", required=True)
